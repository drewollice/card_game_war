...
public enum Player
{
	...
	public Card drawCard ( final boolean revealCard )
	{
		...
		else if ( this.getDeck()
					  .isEmpty() )
		{
			recycle();
		}
		
		final Card drawnCard = this.getDeck()
								   .pop();
	...
	public Collection< Card > drawCards ( final int count,
										  final boolean revealCards )
	{
	...
	public void recycle ()
	{
		...
		while ( ! this.getDiscard()
					  .isEmpty() )
		{
			this.getDeck()
				.push( this.getDiscard()
						   .pop() );
		}
	}
}
