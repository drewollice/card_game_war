...
public class Battle
{
	...
	public Battle (final Player player1, final Player player2)
	{
	...
	public Battle (final Collection<Card> spoilsOfWar, final Player player1, final Player player2)
	{
	...
	public Battle run ()
	{
		...
		{
			return new War(this.spoilsOfWar, this.player1, this.player2).run();
		}
	...
	{
		final int total = this.player1.getDeck().size() + this.player1.getDiscard().size() + this.player2.getDeck().size() + this.player2.getDiscard().size();
		System.out.println("Total cards in game:\t" + total);
		
		System.out.println(this.player1.getName() + " Deck:\t" + this.player1.getDeck().size() + "\tDiscard:\t" + this.player1.getDiscard().size());
	...
	{
		this.spoilsOfWar.forEach(card -> this.winner.getDiscard().push(card));
	}
